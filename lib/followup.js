...
function buildStep(lead, index) {
  ...
  // Texto en crudo: se escapa una sola vez al inyectarlo en el HTML (más abajo).
  // El texto plano y el asunto usan el crudo, que es lo correcto.
  const name = String(lead.name || "").trim().split(/\s+/)[0] || (en ? "there" : "");
  const svc = lead.service || (en ? "your project" : "tu proyecto");
      ...
      <div style="max-width:520px;margin:0 auto;background:#ffffff;border:1px solid #ece7de;border-radius:16px;overflow:hidden;box-shadow:0 2px 6px rgba(20,14,4,.05)">
        ...
        <div style="padding:30px 32px 10px">
          <div style="margin:0 0 20px">
            <img src="https://www.pittahaya.com/assets/pitahaya-logo.png" alt="Pittahaya" width="42" height="42" style="display:inline-block;vertical-align:middle;border:0;width:42px;height:42px">
...
// Inserta una fila (best-effort). No lanza: si la tabla no existe todavía, se ignora.
...
async function runAutoFollowups({ limit = MAX_PER_RUN, force = false } = {}) {
  ...
  for (const lead of leads || []) {
    ...
    try {
      ...
      await supaInsert('followup_log', { lead_id: lead.id, name: lead.name, email: lead.email, country: lead.country, step: idx + 1 });
      sent++;
